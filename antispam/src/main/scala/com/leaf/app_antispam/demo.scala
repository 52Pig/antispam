...
object Antispm extends App {
  val config = new SparkConf().setAppName("Leaf View Detection")
  ...
  for (f <- flist) {
    ...
    // 检测当日概率表是否已经生成
    val resultfileexists = fs.exists(new Path("hdfs:///user/analyst/leaf/"+date+"/like/_SUCCESS"))
    ...
    if (datafileexists && !resultfileexists) {
      ...
      vvfit.repartition(1).sort("os","dev","num").rdd.saveAsTextFile("hdfs:///user/analyst/leaf/"+date+"/vvfit")
      tsfit.repartition(1).sort("os","dev","num").rdd.saveAsTextFile("hdfs:///user/analyst/leaf/"+date+"/tsfit")
      vvtable.unpersist
      tstable.unpersist
      ...
      // 检测前一天的概率大表是否存在
      val likefileexists = fs.exists(new Path("hdfs:///user/analyst/leaf/"+oneDayBefore+"/like/_SUCCESS"))

      if (likefileexists) {
        // 若存在则读取这张概率大表
        val likerdd = sc.textFile("hdfs:///user/analyst/leaf/"+oneDayBefore+"/like/*")
      ...
      likefit.rdd.map(_.mkString(",")).saveAsTextFile("hdfs:///user/analyst/leaf/"+date+"/like")
      datedf.unpersist
    }
  }
}
